...
def _pipeline_cache_try_load(
    ...
    try:
        ...
        if isinstance(exc, NotImplementedError) and "StringDtype" in str(exc):
            hint = "NotImplementedError (pandas StringDtype pickle)"
        logger.info(
            "Removed unreadable pipeline cache %r (%s); recomputing.",
            name,
            hint,
        )
        return _CACHE_MISS
...
def run_loo_clean(
    ...
    *,
    ...
    for _col in df.columns:
        if pd.api.types.is_string_dtype(df[_col]) or isinstance(
            df[_col].dtype, pd.CategoricalDtype
        ):
            df[_col] = df[_col].astype(str).astype(object)
    ...
    for _col in group_summary.columns:
        if pd.api.types.is_string_dtype(group_summary[_col]) or isinstance(
            group_summary[_col].dtype, pd.CategoricalDtype
        ):
            group_summary[_col] = group_summary[_col].astype(str).astype(object)
