...
\sloppy
We list each member's primary responsibilities below. Where both names appear, the work was paired. Concrete file and figure references are given so the division of labor can be audited against the repository.

\paragraph{Djordje Ivanovic.}
\begin{itemize}
\setlength{\itemsep}{1pt}
    \item \emph{Code (lead).} \texttt{logic/pipeline.py}, \texttt{logic/decoding.py}, \texttt{logic/rdm.py}, \texttt{logic/analyses.py}, \texttt{logic/subsets.py}, \texttt{logic/stats.py}, \texttt{logic/bellier\_data.py}, \texttt{logic/bellier\_decoder.py}, \texttt{logic/nonlinear.py}, \texttt{logic/temporal\_profile.py}, \texttt{logic/plots.py}, \texttt{logic/cache\_io.py}, \texttt{logic/config.py}.
    \item \emph{Analyses.} Matched 7-vs-7 random-subset control; acoustic partialling and shuffle-correct null; cross-temporal generalisation; LOO; Bellier supergrid + interleaved-folds CV; non-linear controls.
    \item \emph{Figures and tables.} \texttt{fig1\_main\_composite}, \texttt{fig2\_mechanisms\_composite}, \texttt{fig3\_bellier\_composite}, \texttt{fig\_nonlinear\_comparison}; Tables~\ref{tab:methods-roadmap}, \ref{tab:acoustic}, \ref{tab:loo}, \ref{tab:bellier}, \ref{tab:profiles}.
    \item \emph{Writing.} Methods (\S\ref{sec:methods}), Results (\S\ref{sec:results}), most of Discussion (\S\ref{sec:discussion}).
\end{itemize}

\paragraph{Georgia Hutchinson.}
\begin{itemize}
\setlength{\itemsep}{1pt}
    \item \emph{Code.} [fill: which \texttt{logic/} modules or notebook cells; e.g.\ refinements to \texttt{decoding.py} folds, \texttt{nonlinear.py} grid, plotting helpers in \texttt{plots.py}].
    \item \emph{Analyses.} Modelling and validation of the song/speech/music decoder; presentation of results [fill: any specific subsections owned end-to-end].
    \item \emph{Figures and tables.} [fill: e.g.\ \texttt{fig1\_dataset\_overview}, confusion-matrix panel, table styling].
    \item \emph{Writing.} [fill: e.g.\ Methods \S2.3--2.5, Results \S3.1--3.3, integration of figure captions].
\end{itemize}

\paragraph{JaKayla Harris.}
\begin{itemize}
\setlength{\itemsep}{1pt}
    \item \emph{Research survey and framing.} Literature scan of song/music selectivity in human auditory cortex; mapping prior work to the gap addressed here.
    \item \emph{Writing.} Introduction (\S\ref{sec:intro}); references; non-specialist clarity passes [fill: any further sections drafted, e.g.\ Discussion paragraph on broader implications].
    \item \emph{Other.} [fill: e.g.\ proposal/check-in drafts, peer-review response triage, glossary additions].
\end{itemize}

\paragraph{Joint work.}
All three members jointly scoped the research questions, agreed on the matched random-subset control and the acoustic-partialled extension, integrated peer-review feedback, and reviewed every figure, table, and headline number in this manuscript before submission.
...
\sloppy
This appendix documents how generative AI was used in the project, what we verified ourselves, and where AI suggestions made it into the final write-up. We did not use AI to invent results; numbers reported in the paper come from the pipeline output in \texttt{results/tables/*.csv}.

\paragraph{Tools used.}
\begin{itemize}
\setlength{\itemsep}{1pt}
    \item \emph{Cursor IDE} with Anthropic Claude (4.x) and OpenAI GPT (5.x) as in-editor assistants for code, LaTeX, and prose review.
    \item [fill: any other tool actually used, e.g.\ ChatGPT-web for brainstorming, GitHub Copilot for autocomplete, Claude.ai for long-context reads]. Remove this line if not applicable.
\end{itemize}

\paragraph{Tasks where AI assisted.}
\begin{itemize}
\setlength{\itemsep}{1pt}
    \item \emph{Number/figure synchronisation.} Cross-checking the random-subset control numbers in \S\ref{sec:randsubset} against \texttt{results/tables/random\_subset\_control\_summary.csv}, which surfaced an earlier draft mistake (the no-song reference $0.885$ had been printed as if it were the song-only score). The corrected $p$-values ($0.63$ for bacc, $0.35$ for divergence) and the $\approx 0.840$ song-only bacc were re-read directly from the CSV and confirmed against \texttt{logic/stats.py::empirical\_p\_value} before the writeup was edited.
    \item \emph{Prose drafting and tightening.} Initial drafts of the methods roadmap (Table~\ref{tab:methods-roadmap}), the Bellier ``Vocal annotation and limitations'' paragraph (\S\ref{sec:methods:bellier}), and the ``Selectivity versus information'' paragraph in Discussion. Each paragraph was then revised by hand for accuracy, scope, and tone.
    \item \emph{Glossary / non-specialist passes.} One-sentence glosses for RDM, balanced accuracy, and cross-temporal generalisation on first use; first-use expansions for STG, HFA, CV, ROC-AUC, CNN, MLP, and RBF.
    \item \emph{LaTeX hygiene.} Resolving \texttt{Overfull \textbackslash hbox} warnings for the roadmap table (using \texttt{resizebox}); \texttt{latexmk} dependency reasoning when bibliography references appeared undefined.
    \item \emph{Caveats and future-directions phrasing.} Wording for the Caveats list (\S\ref{sec:discussion}) and for the conservative interpretation of the acoustic-partialled residual.
    \item [fill: any other task we used AI for, e.g.\ debugging a specific function, suggesting a CV scheme, alternative model brainstorming, ICA labelling].
\end{itemize}

\paragraph{What we verified ourselves.}
\begin{itemize}
\setlength{\itemsep}{1pt}
    \item Re-ran \texttt{python logic/pipeline.py} (seed 42) and inspected \texttt{results/tables/*.csv}; every numeric claim in the abstract, results, and tables traces to a row in those CSVs.
    \item Read \texttt{compare\_true\_vs\_random\_subsets} (in \texttt{logic/analyses.py}), \texttt{empirical\_p\_value} (in \texttt{logic/stats.py}), and \texttt{logic/rdm.py} to confirm that the $p$-value direction and divergence definition match the prose.
    \item Recompiled \texttt{writeup.pdf} with \texttt{latexmk} and visually checked that figure-panel $p$-annotations match the text.
\end{itemize}

\paragraph{What was misleading or wrong once.}
\begin{itemize}
\setlength{\itemsep}{1pt}
    \item AI-assisted prose initially repeated an older draft's claim that song-only sat at the ``64th percentile'' with $p = 0.36$ on balanced accuracy. The current pipeline output shows song-only \emph{below} the random-draw mean ($p = 0.63$) and only divergence sitting modestly above the mean. We caught this by reading the CSV directly and corrected the abstract, \S\ref{sec:randsubset}, and Discussion.
    \item AI suggested the phrase ``post-acoustic code'' for the partialled residual. We rejected this wording because the test cannot distinguish higher-order coding from variance not captured by the specific linear feature bank.
    \item A draft pass once aligned LOO prose to \texttt{results/metrics.json} instead of \texttt{results/tables/loo\_group\_summary.csv}, overstating group-mean $\Delta$bacc and reporting $p = 0.014$. The manuscript and \texttt{metrics.json} LOO block were corrected to match the CSV ($p \approx 0.23$). Panel annotations on Fig.~\ref{fig:mechanisms} refresh when \texttt{fig7\_loo\_contributions\_*} is regenerated from the pipeline.
\end{itemize}

\paragraph{Reflection.}
AI was most useful for prose tightening, LaTeX hygiene, and surfacing internal inconsistencies between text, tables, and CSVs. It was least useful when asked for ``the answer'' to scientific questions: every claim in the paper was checked against pipeline output before being kept. AI suggestions that survived into the final document are flagged in the relevant tasks above; nothing in the Results section was authored by AI without numerical verification.
