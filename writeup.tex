...
\title{\textbf{Do song-selective electrodes carry more song-vs-music information? \\ A re-analysis of Norman--Haignere et al.\ (2022) with a Bellier et al.\ (2023) extension}}
\author{Djordje Ivanovic, Georgia Hutchinson, JaKayla Harris \\ Group 5, NEURO 120 (Spring 2026)}
\date{\today}
...
\begin{center}
\textbf{Codebase:} \url{https://github.com/djordjeivanovic11/neuro120-project}
\end{center}

\vspace{0.5em}
% =============================================================================
\begin{abstract}
\noindent We asked a simple question: do the brain regions that respond most strongly to song also carry the most \emph{information} a decoder could use to tell song apart from instrumental music? \citet{NormanHaignere2022} identified seven electrocorticography (ECoG) electrodes in human auditory cortex whose average response is stronger to song than to either speech or instrumental music. We re-analysed their dataset (33 electrodes, 49 stimuli, 0--2~s trial windows) and tested whether those seven electrodes actually carry more song-vs-music information than other nearby electrodes do. To make the comparison fair, we drew 7 random electrodes from the non-song pool and measured their decoder score, repeating this 1000 times. If the song electrodes are truly special, their score should beat almost all of these random groups.

They do not. A simple classifier trained on the 7 song electrodes does about as well as classifiers trained on random groups of 7 non-song electrodes ($p = 0.36$), and the same holds for a geometric separation score computed from a representational dissimilarity matrix (RDM, $p = 0.35$). The song electrodes only looked better than the full 33-electrode pool in the raw curves because smaller feature groups are easier for a classifier to fit, so they tend to look artificially better regardless of which electrodes you pick.

We also asked how much of the song-vs-music signal is just low-level acoustics. We subtracted out a bank of basic sound features (a cochleogram plus spectrotemporal-modulation energy) from the neural responses at every time window. Roughly two thirds of the divergence disappears, meaning most of the signal is explained by simple sound properties. About a third of the raw peak survives across all three electrode subsets (fraction surviving at peak: $0.33$ song-only, $0.39$ all, $0.47$ no-song), hinting that the brain is tracking something beyond raw audio. At this sample size, though, the survival is only borderline significant in song-only ($p = 0.052$) and does not clear significance in the other subsets ($p = 0.108$ and $p = 0.410$), so we treat it as suggestive rather than conclusive.

Finally, we tested whether the picture extends to continuous music using the \citet{Bellier2023} Pink-Floyd ECoG dataset. A vocal-vs-instrumental decoder reaches balanced accuracy $\approx 0.78$ on the full supergrid, shows the right-STG $>$ left-STG asymmetry reported by \citet{Bellier2023}, and is highest on the whole electrode pool rather than any one anatomical subset. Taken together: song-selective electrodes are informative but not special, and some of the song-vs-music signal appears not to be linearly predictable from low-level acoustics.
\end{abstract}
...
All decoding uses $L_2$-penalised logistic regression with per-fold standardisation ($C = 1$), the standard choice for cognitive ECoG \citep{Holdgraf2017}. We report \emph{balanced accuracy} (bacc), the mean per-class recall.\footnote{We use balanced rather than plain accuracy because the classes are unequal in size; a lazy classifier that always guesses \textsf{music} would look deceptively good on plain accuracy.} Cross-validation (CV) uses stratified group 5-fold splits with stimulus ID as the grouping variable, repeated 20 times with different random seeds for the headline numbers. Grouping by stimulus is essential: without it, the classifier could cheat by memorising a stimulus it saw during training, inflating accuracy without reflecting class-level information.

\subsection{Divergence: a classifier-free separation score}\label{sec:methods:divergence}
Decoding asks whether a classifier \emph{can} separate the classes. We also wanted a geometric measure, independent of any classifier, that asks how structured the neural responses are in the first place. For this we use a representational dissimilarity matrix (RDM): the matrix of pairwise distances between the neural responses to every pair of stimuli. If category is reflected in neural geometry, pairs of stimuli from different classes should sit farther apart in the RDM than pairs from the same class.
...
On these folds we fit $L_2$ logistic regression with \texttt{LogisticRegressionCV} ($C \in \{10^{-3}, \dots, 10^{3}\}$, inner 3-fold) on the electrode-means of each window. We report both balanced accuracy and the area under the ROC curve (ROC-AUC), the latter being a threshold-free summary of how well the decoder's continuous scores separate the two classes. We only run a more expensive non-linear model on subsets that already show a clear linear signal, so our small CNN (\texttt{TinyTemporalCNN}: two 1-D convolutions, adaptive pooling, linear head, 40 training epochs) is restricted to subsets whose logistic regression clears chance by at least $0.05$ bacc. This avoids tuning a complex model against near-chance features, where apparent gains are most likely to reflect noise. The 40-epoch schedule is a standard short-training budget for a small model on the few thousand windows we have; we did not observe further improvement beyond this.
...
\emph{We now zoom in on the song-vs-music pair specifically: when during the trial is that distinction actually decodable?} The answer: throughout the trial, at essentially every electrode subset (panel~(b) of Fig.~\ref{fig:mainnorman}). Peak balanced accuracy is 1.00 on \textsf{all} and on the 7 song-selective electrodes (\textsf{song\_only}), and 0.95 on the remaining 26 speech- and music-selective electrodes (\textsf{no\_song}). Timing differs across subsets: the full pool (\textsf{all}) peaks at 0.32~s, while \textsf{no\_song} and \textsf{song\_only} both peak later at 0.67~s. The cleanest thing to say about the full-pool early peak is that it relies on combining information across all 33 electrodes; neither subset on its own reaches a decoder peak that early. These timings are consistent with a contribution from early-onset electrodes in the STG onset-vs-sustained spatial map of \citet{Hamilton2018}. The decoder clearly does not depend on song-selective electrodes to reach chance-clearing performance: \textsf{no\_song} still reaches 0.95. Whether \textsf{song\_only} outperforms \textsf{no\_song} in a meaningful way is exactly the question the random-subset control in \S\ref{sec:randsubset} is designed to answer.
...
Song-only is not special under this fair comparison (panels~(d--e) of Fig.~\ref{fig:mainnorman}). On balanced accuracy it sits at the 64th percentile of the 1000 random draws (0.885, $p = 0.36$), and at a similar percentile on divergence (0.236, $p = 0.35$).

Two numbers in this result might seem to disagree at first, so it is worth walking through what each one means. We look at both the \emph{tail} of the random-draw distribution (via the $p$-value) and the \emph{average} of that distribution (via a bootstrap 95\% CI on the song-only minus random-draw mean). The $p$-values ($0.36$, $0.35$) answer: \emph{does the song-only score lie in the top few percent of random-group scores?} The answer is no; it sits near the middle. The bootstrap CI for song-only minus random-draw mean is strictly positive (about $+0.03$ on bacc, $+0.04$ on divergence), which answers a different question: \emph{is the song-only score above the average of the random draws?} The answer is yes, very slightly. Both can be true: there is a small per-electrode advantage on song-selective electrodes, but it is well within what any 7 randomly chosen non-song electrodes can produce. This is our central negative result: song electrodes as a group do not carry disproportionately more song-vs-music information than neighbouring auditory-cortex electrodes.
...
The song-selective subset shows a somewhat more temporally stable representation than the non-song pool (panel~(b) of Fig.~\ref{fig:mechanisms}). Diagonal vs.\ off-diagonal-at-$\geq 0.25$~s balanced accuracy is $0.92/0.84$ for \textsf{all}, $0.92/0.83$ for \textsf{song\_only}, and $0.87/0.76$ for \textsf{no\_song}: a decoder trained at 0.3~s still works at 1.5~s on the song electrodes, and less so on the non-song pool. We do not have a permutation null on this comparison, so we read the $\approx$8-point vs.\ $\approx$11-point off-diagonal gaps as suggestive of a sustained categorical code on the song electrodes rather than a formal significant result.

\subsection{Per-electrode contributions (leave-one-out)}

\emph{The random-subset control in \S\ref{sec:randsubset} tests song electrodes as a group. Here we run a complementary test at the single-electrode level, asking whether removing any one electrode disproportionately hurts the decoder, and whether those important electrodes concentrate in the song group.} Song-labeled electrodes show a slightly higher mean drop in time-averaged balanced accuracy when removed than the other groups (panels~(c--d) of Fig.~\ref{fig:mechanisms}; Table~\ref{tab:loo}). Mean $\Delta$bacc is $+0.0053$ for song, $+0.0030$ for music, and $+0.0020$ for speech, with a song-vs-rest permutation $p = 0.014$ (1000 shuffles). The magnitudes are small and the distributions overlap, so we read this as a modest single-electrode tilt, not a second ``special population'' claim: the fair 7-vs-7 matched subset test in \S\ref{sec:randsubset} remains the primary evidence that the seven song contacts as a \emph{set} are not unusually informative relative to other size-matched non-song draws.
...
\midrule
song   &  7 & $+0.0053$ & $+0.0017$ & 0.014 \\
music  & 11 & $+0.0030$ & $+0.0049$ & -- \\
speech & 15 & $+0.0020$ & $-0.0039$ & -- \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{A concern with our negative random-subset result is that it might be an artefact of using a linear decoder: maybe a non-linear model could extract much more song-vs-music information from the song electrodes.} Non-linear models do not beat the linear baseline on this dataset (Fig.~\ref{fig:nonlinear}). Neither an RBF-SVM nor an autoencoder-latent + logreg pipeline improves on logistic regression: $\Delta$bacc $= 0$ for song-vs-music and $\Delta$bacc $\in \{-0.046, 0\}$ for the three-class task. At this sample size and feature dimension, a simple linear decoder already extracts most of the available signal, so adopting a linear decoder throughout the paper does not seem to be leaving obvious non-linear information on the table. This is consistent with the STG literature on low-dimensional feature tuning \citep{Mesgarani2014,Hullett2016}.

\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{fig_nonlinear_comparison.pdf}
\caption{\textbf{Non-linear supplement.} Grouped CV balanced accuracy of three model families on two tasks (three-class and song-vs-music, all-electrodes) using identical stimulus-grouped 5-fold splits. Error bars are across-fold standard deviations. Neither the RBF-SVM nor the autoencoder-latent pipeline materially outperforms the linear logistic baseline.}
\label{fig:nonlinear}
\end{figure}
...
\begin{table}[t]
\centering\small
\caption{\textbf{Bellier (2023) vocal-vs-instrumental decoder.} $1903$ windows, 500~ms / 100~ms, interleaved 5-fold blocked-time CV. CNN is run only on subsets where logreg clears chance by $\geq 0.05$.}
\label{tab:bellier}
\begin{tabular}{lrrrrr}
\toprule
subset & $n_\text{elec}$ & logreg bacc & logreg AUC & CNN bacc & CNN AUC \\
\midrule
all        & 2395 & \textbf{0.777} & 0.898 & 0.765 & 0.839 \\
right\_STG &  181 & 0.692 & 0.814 & \textbf{0.733} & 0.839 \\
left\_STG  &  245 & 0.684 & 0.780 & 0.723 & 0.806 \\
non\_STG   & 1969 & 0.730 & 0.892 & 0.731 & 0.825 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Code and data availability.} All analyses run from the \texttt{logic/} directory (\texttt{python pipeline.py}, random seed 42) or via \texttt{python scripts/generate\_composite\_figures.py} from the repository root for the multi-panel figures (see \texttt{--help}, including \texttt{--only 3} for the Bellier composite alone). The full codebase, including the notebook used to generate every figure and table, is available at \url{https://github.com/djordjeivanovic11/neuro120-project}. A separate document states course policy on generative-AI use. Norman--Haignere data are from the public release accompanying \citet{NormanHaignere2022}; Bellier ECoG data are from the public release accompanying \citet{Bellier2023}.
...
\begin{itemize}
\setlength{\itemsep}{1pt}
    \item \textbf{Djordje Ivanovic.}
    Code and modeling

    \item \textbf{Georgia Hutchinson.}
    modeling and presentation of results, methods

    \item \textbf{JaKayla Harris.}
    Introductory research, research survey
\end{itemize}

All three members jointly scoped the research questions, agreed on the matched random-subset control and the acoustic-partialled extension, and reviewed every figure and table in this manuscript.
